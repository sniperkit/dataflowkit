...
PROG_NAME := "dataflowkit"

all: deps-ensure test build install version

build: deps-ensure
	@go build -ldflags "-X main.VERSION=`cat VERSION`" -o ./bin/df-fetch.cli ./cmd/fetch.cli/*.go
	@go build -ldflags "-X main.VERSION=`cat VERSION`" -o ./bin/df-fetch.d ./cmd/fetch.d/*.go
	@go build -ldflags "-X main.VERSION=`cat VERSION`" -o ./bin/df-parse.d ./cmd/parse.d/*.go
